import * as React from 'react'
import { cva, type VariantProps } from 'class-variance-authority'
import { cn } from '../../lib/utils'

export interface FooterLink {
  label: string
  href: string
}

export interface FooterColumn {
  title: string
  links: FooterLink[]
}

export interface FooterSocial {
  platform: string
  href: string
}

const sectionCva = cva('w-full', {
  variants: {
    variant: {
      light: 'bg-[--si-body-bg] border-t border-[--si-border-color]',
      dark: 'bg-[--si-dark]',
    },
  },
  defaultVariants: { variant: 'dark' },
})

const headingCva = cva('mb-4 text-sm font-semibold uppercase tracking-widest', {
  variants: {
    variant: {
      light: 'text-[--si-heading-color]',
      dark: 'text-white',
    },
  },
  defaultVariants: { variant: 'dark' },
})

const linkCva = cva(
  'text-sm transition-colors duration-150 hover:text-[--si-primary]',
  {
    variants: {
      variant: {
        light: 'text-[--si-body-color]',
        dark: 'text-[--si-gray-400]',
      },
    },
    defaultVariants: { variant: 'dark' },
  },
)

const taglineCva = cva('mt-2 text-sm', {
  variants: {
    variant: {
      light: 'text-[--si-body-color]',
      dark: 'text-[--si-gray-400]',
    },
  },
  defaultVariants: { variant: 'dark' },
})

const bottomBarCva = cva('border-t py-6 text-sm', {
  variants: {
    variant: {
      light: 'border-[--si-border-color] text-[--si-body-color]',
      dark: 'border-white/10 text-[--si-gray-400]',
    },
  },
  defaultVariants: { variant: 'dark' },
})

const socialIconMap: Record<string, string> = {
  twitter: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z',
  github:
    'M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z',
  linkedin:
    'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z',
  facebook:
    'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z',
  instagram:
    'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z',
  youtube:
    'M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z',
}

function SocialIcon({ platform, href }: FooterSocial) {
  const d = socialIconMap[platform.toLowerCase()]

  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={platform}
      className="flex h-9 w-9 items-center justify-center rounded-full bg-white/5 text-[--si-gray-400] transition-all hover:bg-[--si-primary] hover:text-white"
    >
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden="true">
        {d ? <path d={d} /> : <circle cx="12" cy="12" r="10" />}
      </svg>
    </a>
  )
}

export type FooterSectionVariant = 'light' | 'dark'

export interface FooterSectionProps
  extends React.HTMLAttributes<HTMLElement>,
    VariantProps<typeof sectionCva> {
  logo?: string
  tagline?: string
  columns: FooterColumn[]
  social?: FooterSocial[]
  copyright: string
}

const FooterSection = React.forwardRef<HTMLElement, FooterSectionProps>(
  (
    { className, variant = 'dark', logo, tagline, columns, social, copyright, ...props },
    ref,
  ) => (
    <footer ref={ref} className={cn(sectionCva({ variant }), className)} {...props}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 gap-8 py-12 lg:grid-cols-5 lg:py-16">
          {/* Brand column */}
          <div className="col-span-2 lg:col-span-1">
            {logo && (
              <img src={logo} alt="Logo" className="mb-4 h-8 w-auto" />
            )}
            {!logo && (
              <div className="mb-4 text-lg font-bold text-[--si-primary]">Silicon</div>
            )}
            {tagline && <p className={cn(taglineCva({ variant }))}>{tagline}</p>}

            {social && social.length > 0 && (
              <div className="mt-6 flex gap-3">
                {social.map((s) => (
                  <SocialIcon key={s.platform} {...s} />
                ))}
              </div>
            )}
          </div>

          {/* Link columns */}
          {columns.map((col) => (
            <div key={col.title}>
              <h3 className={cn(headingCva({ variant }))}>{col.title}</h3>
              <ul className="space-y-3">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <a href={link.href} className={cn(linkCva({ variant }))}>
                      {link.label}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        <div className={cn(bottomBarCva({ variant }))}>
          <p>{copyright}</p>
        </div>
      </div>
    </footer>
  ),
)
FooterSection.displayName = 'FooterSection'

export { FooterSection }
